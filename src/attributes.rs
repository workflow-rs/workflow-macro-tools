    ...
    fn parse(input: ParseStream) -> syn::Result<Self> {
        input.step(|cursor| {
            if let Some((ident, rest)) = cursor.ident() {
                let wraper = IdentWraper(
                    proc_macro2::Ident::new(&ident.to_string(), ident.span())
                );
...
impl Args {
    ...
    pub fn new() -> Args { Args { map: HashMap::new() } }
                ...
                match token {
                    ...
                    Item::String(lit_str) => {
                        ...
                        if map.get(&default).is_none() {
                            map.insert(default, Some(Value::EvaluationValue(EvaluationValue::String(lit_str))));
                        }
                    },
                    _ => {
